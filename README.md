...
| `ParameterNameRegex` | Имя параметра из `@params` должно совпасть с regex, например `@P\\d+` для `@P1`, `@P2`, ... |
| `ParameterType` | Тип параметра из `@params` должен совпасть с указанным значением, например `datetime2(3)`. Требует `ParameterExists` или `ParameterNameRegex`. |
...
| `SetParameterType` | `SqlType`, `Name` (опционально) | Меняет тип параметра `sp_executesql`, например `datetime2(0)`. Если `Name` не указан, действие применяется ко всем параметрам, прошедшим фильтр `When`. |

При загрузке конфига включенные правила валидируются: `SetParameterValue` требует `Value`, `SetParameterType` требует `SqlType`, parameter actions требуют `Name` или parameter filter в `When`, `ParameterType` требует `ParameterExists` или `ParameterNameRegex`.
...
### Фильтр по таблице, `@Pn` и `datetime2(3)`

Одно правило может применяться ко всем параметрам `@P1`, `@P2`, ... с типом `datetime2(3)` в запросах к конкретной таблице:
...
Изменятся только `@P1` и `@P2`; `@P3` с типом `int` не затрагивается.
...
- что для `RpcSpExecuteSql` parameter filter в `When` (`ParameterExists`, `ParameterNameRegex`, `ParameterType`) совпадает с объявлением в `@params`, а `Actions[].Name` либо указан явно, либо опущен для matched parameters;
